  ...
  useEffect(() => {
    const fetchMapData = async () => {
      try {
        ...
        console.log("Map data loaded:", data);  // Log the loaded data to verify it's correct
        setMapData(data);
      } catch (error) {
              ...
              <div style={{ height: '500px' }}>
                {(() => {
                  if (mapData) {
                    ...
                    options={{
                      chart: {
                        map: mapData,  // The map data you've fetched
                      },
                      title: {
                        text: 'World Map',
                      },
                      subtitle: {
                        text: 'High resolution world map',
                      },
                      mapNavigation: {
                        enabled: true,
                      },
                      colorAxis: {
                        min: 0,  // Min value for color axis
                        max: 100,  // Max value for color axis (adjust as needed)
                      },
                      series: [{
                        type: 'map',
                        mapData: mapData,
                ...
                <div id="sku-map-container" style={{ height: '500px' }}></div>  {/* Add div to render the map */}
              </div>
            )}
          </SKU_Rationalization_Tab>
